...
export function Sidebar() {
  ...
  const customerClaims = currentCustomer
    ? claims.filter((claim) => claim.customerId === currentCustomer.id)
    : [];
  const customerPolicies = currentCustomer
    ? policies.filter((policy) => policy.customerId === currentCustomer.id)
    : [];
  ...
  return (
    <aside className="relative flex h-[100dvh] w-[280px] shrink-0 flex-col overflow-hidden bg-card border-r-2 border-foreground">
      <div className="shrink-0 px-5 py-4 border-b-2 border-foreground">
        <div className="flex items-center justify-between gap-2.5">
          <div className="flex min-w-0 items-center gap-2.5">
            <div className="w-9 h-9 shrink-0 bg-accent rounded-[10px] border-2 border-[#6D28D9] shadow-[2px_2px_0_#6D28D9] flex items-center justify-center text-white font-heading font-800 text-sm">
              CC
            </div>
      ...
      <nav className="shrink-0 px-3 py-2" aria-label="Primary navigation">
        {navItems.map((item) => (
      ...
      <div className="mx-4 shrink-0 border-t-2 border-border my-1" />
      ...
      {profileOpen && currentCustomer && (
        <div className="fixed inset-0 z-[60]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setProfileOpen(false); }}>
          <section role="dialog" aria-modal="true" aria-label="User profile" className="absolute bottom-4 left-4 w-[calc(100vw-2rem)] max-w-[360px] animate-pop-in rounded-[var(--radius-md)] border-2 border-foreground bg-card p-4 shadow-[6px_6px_0_var(--foreground)] sm:left-4">
            ...
            <div className="grid grid-cols-1 gap-2.5 pt-3 text-xs">
              ...
              <ProfileRow label="Active policies" value={String(customerPolicies.length)} />
              <ProfileRow label="Claims" value={String(customerClaims.length)} />
            </div>
          </section>
        </div>
      )}
