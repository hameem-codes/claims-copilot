...
export function AppProvider({ children, user }: { children: ReactNode; user?: import("@supabase/supabase-js").User | null }) {
  const authCustomer: Customer | null = user ? {
    id: user.id,
    name: (user.user_metadata?.full_name as string) || user.email?.split("@")[0] || "User",
    email: user.email || "",
    phone: user.phone || "",
    customerSince: user.created_at ? new Date(user.created_at).toLocaleDateString("en-US", { month: "short", year: "numeric" }) : new Date().getFullYear().toString(),
  ...
  const currentCustomer = selectedCustomer || authCustomer;
  ...
  const createConversation = useCallback(() => {
    const customerId = currentCustomer?.id || "user";
